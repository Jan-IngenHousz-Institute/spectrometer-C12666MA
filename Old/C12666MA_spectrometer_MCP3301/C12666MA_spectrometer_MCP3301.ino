 ...
 *
 *  --- PROTOCOL VERIFICATION WARNING ---------------------------------------------
 *     The SPI bit-extraction in readMCP3301() below was built by extending
 *     the well-documented MCP3201 (12-bit, same chip family) read pattern
 *     by one bit to account for the MCP3301's extra sign bit - it was NOT
 *     verified against a physical device or the datasheet's own timing
 *     figure (DS21700), which this tool could not directly inspect.
 *     BEFORE trusting this for real measurements: feed IN+ a known, stable
 *     DC voltage (e.g. from a potentiometer between VDD and GND) and
 *     confirm readMCP3301() reports a value that scales sensibly and
 *     linearly as you adjust it, across the full range including near 0V
 *     and near VREF. Do this BEFORE reconnecting the sensor.
 *
...
// Safety cap for integration time to avoid extremely long or runaway captures
const unsigned long MAX_INTEGRATION_US = 10000000UL; // 10 seconds
...
// hold MAX_AVG * 4095 without overflow. On an Arduino Uno the available
// SRAM is limited (2KB total), so using a large accumulator array of
// uint32_t for long averages can exhaust memory and cause stack/heap
// collisions. To balance resolution and memory use, cap averaging at 15
// frames so a uint16_t accumulator is sufficient (15 * 4095 = 61425 < 65535)
// while keeping acquisition time reasonable.
...
uint16_t g_data[SPEC_CHANNELS];   // latest frame accumulator (fits up to MAX_AVG * 4095 when MAX_AVG <= 15)
                                   // using uint16_t halves RAM usage compared with uint32_t
                                   // and prevents SRAM exhaustion on AVR Uno-class boards.
...
void loadConfig() {
  ...
  if (g_cfg.integration_us > MAX_INTEGRATION_US) g_cfg.integration_us = MAX_INTEGRATION_US;
}
...
// 13-bit result (-4096..+4095). See the PROTOCOL VERIFICATION WARNING at
// the top of this file - this bit extraction was extended from the
// well-documented MCP3201 (12-bit) pattern by one bit to account for the
// MCP3301's extra sign bit, but has NOT been verified against a physical
// device. Test with a known DC voltage before trusting it.
//
// Two bytes (16 SPI clocks) are transferred; the ADC's useful data spans
// bits within those 16 clocks as follows (MSB-first):
//   byte1 bits [7:6] = don't-care/leading bits (discarded)
//   byte1 bits [5:0] = data bits [12:7]  (6 bits, includes the sign bit)
//   byte2 bits [7:1] = data bits [6:0]   (7 bits)
//   byte2 bit  [0]   = padding (discarded)
// giving 6 + 7 = 13 total data bits, sign-extended below since the result
// is two's complement.
int16_t readMCP3301() {
  ...
  int16_t raw = ((int16_t)(hiByte & 0x3F) << 7) | (loByte >> 1);
...
void handleCommand(char *cmd) {
  ...
  if (strcmp(tok, "hello") == 0) {
    ...
    if (arg != NULL) {
      ...
      if (requested < MIN_INTEGRATION_US) requested = MIN_INTEGRATION_US;
      if (requested > MAX_INTEGRATION_US) requested = MAX_INTEGRATION_US;
      g_cfg.integration_us = requested;
      saveConfig();
    }
